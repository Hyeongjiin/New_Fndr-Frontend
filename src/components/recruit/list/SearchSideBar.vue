...
export default {
  ...
  methods: {
    ...
    updateContinentCheckbox(continent) {
      const allChecked = continent.countries.every(
        (country) => this.checkedCountries[country.id + "-" + continent.name]
      );
  ...
  computed: {
    computedCheckedContinents: {
      get() {
        let result = {};
        for (let continent of this.nationsList) {
          result[continent.name] = continent.countries.every(
            (country) =>
              this.checkedCountries[country.id + "-" + continent.name]
          );
        }
        return result;
      },
      set(newValue) {
        for (let continentName in newValue) {
          ...
          const continent = this.nationsList.find(
